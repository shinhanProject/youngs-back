    ...
    public ResponseEntity<?> editSummary(Long userSeq, SummaryDTO summaryDTO) {
        try {
            ...
            if (category.equals("basic")) { // 수정할 요약의 카테고리가 '기초 지식'일 경우
                BasicSummary basicSummary = basicSummaryRepository.findById(summarySeq) // 수정할 기초 지식 요약
                        .orElseThrow(() -> new RuntimeException("수정할 기초 지식 요약이 없습니다.")); // 요약 작성이 되어 있지 않을 경우
